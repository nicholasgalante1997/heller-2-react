import React, { Profiler, ProfilerOnRenderCallback, useState, useEffect, useCallback, useRef, ElementRef } from 'react';
import './RenderMetrics.css';

type RenderMetricsProps = {
...
export function RenderMetricsComponent({ children, id }: RenderMetricsProps) {
  ...
  const onRender: ProfilerOnRenderCallback = useCallback((id, phase, actualDuration, baseDuration, startTime, commitTime, interactions) => {
  ...
  return (
    <div className={classNames.Container}>
      <div className={classNames.ComponentContainer}>
        <Profiler onRender={onRender} id={id}>{children}</Profiler>
      </div>
